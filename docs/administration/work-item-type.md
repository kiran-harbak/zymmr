...
description: A guide for administrators on how to create, customize, and manage Work Item Types like 'Task', 'Bug', and 'Sub-Task' in Zymmr.
head:
  - - meta
    - name: keywords
      content: work item type, issue type, customize, parent child, sub-task, administration, zymmr
outline: deep
---
...
::: tip Understanding Child Types
The **Is Child Type** setting is crucial for hierarchy.
-   **Unchecked (Parent)**: Use this for standard items like `Stories`, `Bugs`, or `Epics` that live at the top level.
-   **Checked (Child)**: Use this for breakdown items like `Sub-Tasks` or `Technical Tasks` that must always live *inside* another work item.
:::
...
4.  Make your desired changes to the **Title**, **Description**, **Is Child Type** status, or **Avatar**.
...
3.  A new, independent work item type will be created.
